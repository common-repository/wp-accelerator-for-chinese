...
if (!class_exists('WP_AcceleratorForChinese')) {
	class WP_AcceleratorForChinese {
		...
		public function __construct() {
			...
			add_filter('get_avatar', array(__CLASS__, 'change_avatar_source'));


		}

		public static function change_avatar_source($avatar) {
			$avatar = str_replace( array('www.gravatar.com','secure.gravatar.com', '0.gravatar.com', '1.gravatar.com', '2.gravatar.com'), 'cn.gravatar.com', $avatar);
			return $avatar;
		}
		...
		public function plugins_loaded() {
			...
			if ($this->o['yahei']) {
				...
				add_action('login_head', array(__CLASS__, 'change_default_font'), 999);
				add_action('admin_head', array(__CLASS__, 'change_default_font'), 999);
				add_action('wp_head', array(__CLASS__, 'change_default_font'), 999);
        ...
        public static function set_bingbg_for_login(){
            ...
            if($img_url&&filter_var($img_url, FILTER_VALIDATE_URL )){
                ...
                if(!is_wp_error($resp)&&is_array($resp)){
                    ...
                    set_transient('bing_image',$img_url,HOUR_IN_SECONDS*4);
                }
            ...
            echo '<style type="text/css">body{background: url("' . $img_url  . '");width:100%;height:100%;background-image:url("' . $img_url  . '");-moz-background-size: 100% 100%;-o-background-size: 100% 100%;-webkit-background-size: 100% 100%;background-size: 100% 100%;-moz-border-image: url("' . $img_url  . '") 0;background-repeat:no-repeat\9;background-image:none\9;}</style>';
